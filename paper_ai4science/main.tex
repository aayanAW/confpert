...
\caption{ConfPert evaluates single-cell perturbation predictors by finite-sample conformal coverage on distributional discrepancies. Left to right: eight predictors (Mean and a bilinear ridge through scGen, CPA, sVAE+, biolord, GEARS, and STATE-600M; $\sim 0$ to $6 \times 10^8$ parameters) on five datasets under three split types produce predicted versus observed cell populations; six distributional discrepancies (KS, Wasserstein-1, energy, MMD-RBF, bimodality match, variance ratio) are computed on each pair; four off-the-shelf conformal heads (per-gene CD-/HPD-split and CQR, per-perturbation split-conformal, per-population OT-CP, subgroup-conditional Mondrian) calibrate a coverage threshold $\hat{\tau}$ at target miscoverage $\alpha$; and three pre-registered claims read off the shared substrate: a calibration benchmark (K1), an honest-null capacity hypothesis (K2), and PRISM pathway enrichment above a random-gene null (K3). The full protocol --- hypotheses, permutation-test thresholds, and multiple-comparison corrections --- is git-stamped before any first run.}
\label{fig:overview}
\end{figure*}
...
The Energy discrepancy collapses to $0$ achieved coverage across all six predictors, and the lightweight predictors collapse on all four within-population shape metrics (KS, Wasserstein-1, energy, MMD-RBF): their unweighted calibration set is computed on K562 scores that systematically under-estimate the K562 $\rightarrow$ RPE1 transfer score. Because the procedure is not exchangeability-valid under this shift, no individual achieved-coverage value is interpretable as calibration; we report the table only to show that the undercoverage is heterogeneous rather than uniform. The heavier predictors do not all collapse to zero --- scGen reaches $0.914$ on bimodality and $0.743$ on the variance ratio, and CPA reaches $0.714$ on bimodality --- but these numbers sit under an invalid procedure, so we draw no architecture-robustness conclusion from them. biolord collapses at $\alpha = 0.20$ (bimodality $0.057$) yet reaches $0.857$ at $\alpha = 0.05$, confirming the values are also $\alpha$-dependent under shift. This heterogeneous undercoverage is the shift-fragility signature analysed by \citet{tibshirani2019}; it motivates a weighted conformal head and a Mondrian-by-cell-line stratification, which we identify as the principled fixes but do not implement here. (sVAE+ is absent because its sampler does not condition on input control cells.)
...
Per-$K$ BH-FDR pass counts at $q = 0.05$ over the joint compound $\times$ Hallmark-pathway grid (10 selective compounds $\times$ 50 Hallmark pathways for the ConfPert path; the uncalibrated baseline is an equal-size random-gene null on the same compound set). $K=50$ is the canonical pre-registered choice; $K=25, 100, 200$ are robustness-check extensions added in this work.

\begin{table}[h]
\caption{K3 top-$K$ robustness sweep. Pre-registered success requires $\geq 3$ enriched ConfPert signatures and $\geq 2$ random-null misses; it holds at every $K$. The ratio column is enrichment-over-null, not a comparison against a competing predictor.}
\label{tab:k3}
\centering
\begin{tabular}{rrrrr}
\toprule
