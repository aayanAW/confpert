...
\subsection{Result: H1 partial positive (2/5); H1 + H1b OVERALL null}
...
\textbf{2 of 5 datasets pass the pre-registered threshold (Replogle RPE1: $\rho = +0.756$, $p = 0.012$; Tahoe-100M: $\rho = +0.707$, $p = 0.033$); H1 OVERALL FAILS} (need $\geq 3$ of 4). Norman near-passes ($p = 0.057$, just above threshold). H1b cross-dataset $\rho = -0.400$, $p = 0.258$, NULL. \emph{All three non-K562 contexts trend H1-positive (RPE1 and Tahoe pass, Norman near); both K562-derived contexts (Replogle K562 at $\rho = +0.10$, Adamson at $\rho = -0.31$) do not.} The cell-line context appears to be the load-bearing covariate for whether capacity hurts calibration. Adding STATE 600M (the largest pre-registered predictor) is what lifts Tahoe-100M over the pre-registered threshold: without STATE the Tahoe trend is $\rho = +0.559$, $p = 0.105$ ($n=7$, sub-threshold), so the second pass depends on the largest model and we report it as such.

\subsection{Honest interpretation}
On Replogle K562 + Adamson, mid-capacity scgen, sVAE+, CPA, biolord all calibrate with lower deviation than tiny baselines (mean, ridge) on most discrepancies, breaking the H1 monotonic capacity-hurts trend. On Replogle RPE1 (the non-cancer cell line) and Tahoe-100M, the trend IS monotonic and significant; on Norman the trend is monotonic but $p$ sits above the pre-reg threshold. Calibration-vs-capacity is dataset-specific, confounded by training-objective family + architecture; H1 OVERALL fails per pre-reg, but the RPE1 + Norman + Tahoe directional consistency on non-K562 contexts is a citable, replication-worthy finding. Figure~\ref{fig:k2h1} visualises the per-dataset scatter.
...
Pre-registration locks four disposition rules for K2 outcomes. The observed result (2 of 5 datasets pass H1; H1b cross-dataset null) maps to disposition (D), \emph{publish honestly with mixed result}. Per pre-registration L160-L161:
